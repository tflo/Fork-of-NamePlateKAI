...
local CASTBAR_TYPES_COLOR = {
	filling = {1.0, 0.7, 0.0}, -- casting
	full = {0.0, 1.0, 0.0}, -- success
	failed = {1.0, 0.0, 0.0}, -- interrupted
}
CASTBAR_TYPES_COLOR[0] = CASTBAR_TYPES_COLOR.filling

local KaiCastBarPrototype = {};
addon.KaiCastBarPrototype = KaiCastBarPrototype
addon.CASTING_BAR_TYPES = CASTING_BAR_TYPES

function KaiCastBarPrototype:InitScripts(...)
	for i = 1, select("#", ...) do
		local scriptName = select(i, ...)
		if self[scriptName] then
			self:SetScript(scriptName, self[scriptName])
		end
	end
end

function KaiCastBarPrototype:SetCastBarTexture(castType)
	if self.barType == "uninterruptable" then
		self:SetStatusBarColor(1, 1, 1)
		self.background:SetAtlas("ui-castingbar-background")
		self.background:SetVertexColor(0.7, 0.7, 0.7, 1.0)
		self:SetStatusBarTexture("ui-castingbar-uninterruptable")
	else
		self:SetStatusBarColor(unpack(CASTBAR_TYPES_COLOR[castType or 0]))
		self.background:SetTexture(addon.db.castBarTexture)
		self.background:SetVertexColor(0.2, 0.2, 0.2, 0.85)
		self:SetStatusBarTexture(addon.db.castBarTexture)
	end
end

function KaiCastBarPrototype:OnLoad(unit, showTradeSkills, showShield)
	self.StagePoints = {};
	self.StagePips = {};
	self.StageTiers = {};
...
function KaiCastBarPrototype:UpdateShownState(desiredShow)
...
function KaiCastBarPrototype:AddWidgetForFade(widget)
...
function KaiCastBarPrototype:SetUnit(unit, showTradeSkills, showShield)
...
function KaiCastBarPrototype:OnShow()
	if ( self.unit ) then
...
function KaiCastBarPrototype:GetEffectiveType(isChannel, notInterruptible, isTradeSkill, isEmpowered)
...
function KaiCastBarPrototype:GetTypeInfo(barType)
	if not barType then
		barType = "standard";
	end
...
function KaiCastBarPrototype:OnEvent(event, ...)
	local arg1 = ...;
	...
	if ( event == "UNIT_SPELLCAST_START" ) then
		...
		self:SetCastBarTexture("filling")
...
function KaiCastBarPrototype:UpdateInterruptibleState(notInterruptible)
...
function KaiCastBarPrototype:OnUpdate(elapsed)
...
function KaiCastBarPrototype:ApplyAlpha(alpha)
	self:SetAlpha(alpha);
...
function KaiCastBarPrototype:FinishSpell()
...
function KaiCastBarPrototype:ShowSpark()
	if ( self.Spark ) then
		self.Spark:Show();
	end
end

function KaiCastBarPrototype:HideSpark()
	if ( self.Spark ) then
		self.Spark:Hide();
	end
end

function KaiCastBarPrototype:PlayInterruptAnims()
...
function KaiCastBarPrototype:StopInterruptAnims()
	if self.HoldFadeOutAnim then
...
function KaiCastBarPrototype:PlayFadeAnim()
...
function KaiCastBarPrototype:PlayFinishAnim()
	if not self.playCastFX then
		return;
	end
...
function KaiCastBarPrototype:StopFinishAnims()
	if self.FlashAnim then
		self.FlashAnim:Stop();
	end
...
function KaiCastBarPrototype:StopAnims()
	self:StopInterruptAnims();
	self:StopFinishAnims();
end

function KaiCastBarPrototype:UpdateIsShown()
...
function KaiCastBarPrototype:SetCastTimeTextShown(showCastTime)
	self.showCastTimeSetting = showCastTime;
	self:UpdateCastTimeTextShown();
end

function KaiCastBarPrototype:UpdateCastTimeTextShown()
...
function KaiCastBarPrototype:UpdateCastTimeText()
	if not self.CastTimeText then
		return;
	end
...
function KaiCastBarPrototype:SetAndUpdateShowCastbar(showCastbar)
	self.showCastbar = showCastbar;
	self:UpdateIsShown();
end

function KaiCastBarPrototype:SetLook(look)
...
function KaiCastBarPrototype:AddStages(numStages)
	...
	local castBarLeft = 0
	local castBarRight = self:GetWidth()
	...
	for i = 1,self.NumStages-1,1 do
		...
		if chargeTier then
			...
			local chargeTierLeft = lastLeft
			local chargeTierRight = chargeTier:GetWidth()
			...
			lastLeft = lastLeft + chargeTierRight
		end
	end

end

function KaiCastBarPrototype:UpdateStage()
	local maxStage = 0;
...
function KaiCastBarPrototype:ClearStages()
